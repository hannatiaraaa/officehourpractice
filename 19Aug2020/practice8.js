...
function printBranch(branches) {
  ...
  const returnArr = []; // | month[0] | Branch-1 IN: branches[0][0][0] | OUT: brancher[0][1][0] | Branch-2 IN: branches[1][0][0] | OUT: 700 | Branch-3 IN: 2000 | OUT: 500 |

  for (let i = 0; i < month.length; i++) {
    // loop bulan => i => 12 kali
    let innerStr = "";
    innerStr += `| ${month[i]} |`; // | jan |
    for (let j = 0; j < branches.length; j++) {
      // loop untuk data (branches) => j = bisa 3 kali atau 1 kali
      innerStr += ` Branch-${j + 1} IN: ${branches[j][0][i]} | OUT: ${
        branches[j][1][i]
      } |`;
    }
    // console.log(innerStr);
    returnArr.push(innerStr);
  }
  return returnArr;
}
